...
Function Set-MyAzureVMJitPolicy{
    [CmdletBinding(DefaultParameterSetName = 'ListParameterSet',
        HelpUri = 'https://go.microsoft.com/fwlink/?LinkID=398573',
        SupportsShouldProcess = $true)]
    Param(
            ...
            If(!$JITexists -or $force)
            {
                ...
                Try{
                    ...
                    Start-AzJitNetworkAccessPolicy -ResourceId "/subscriptions/$SubscriptionID/resourceGroups/$($VM.ResourceGroup)/providers/Microsoft.Security/locations/$($VM.Location)/jitNetworkAccessPolicies/default" `
                                                    -VirtualMachine $JitPolicyArr -ErrorAction Stop
...
Function Enable-MyAzureVMJitPolicy{
    Param(
        [Parameter(Mandatory = $true,
            ValueFromPipeline=$True,
            ValueFromPipelineByPropertyName = $true,
    ...
    Process
    {
        ...
        Foreach($VM in $VMName){
            ...
            $JitPolicy = (@{
                #id="/subscriptions/$($global:MyAzSubscriptionID)/resourceGroups/$($VM.ResourceGroup)/providers/Microsoft.Compute/virtualMachines/$($VM.Name)";
                id="$($VM.Id)";
                ports=(@{
                     number=22;
                     protocol="*";
            ...
            Set-AzJitNetworkAccessPolicy -ResourceGroupName $VM.ResourceGroup -Kind "Basic" -Location $VM.Location -Name "default" -VirtualMachine $JitPolicyArr
        ...
        foreach ($VM in $VMs)
        {
            If( ($VM.JITAccess -ne 'disabled' -or $VM.JITAccess -eq 'False') -and $VM.PublicIP )
            {
                Try{
                    ...
                    Write-host ("...") -ForegroundColor Gray -NoNewline
                    #If($VerbosePreference -eq 'Continue'){Write-Host "Command: Set-MyAzureVMJitPolicy -VMName $($VM.VMName) -NoStatus" -ForegroundColor Yellow}
                    $jitaccess = Set-MyAzureVMJitPolicy -VMName $VM.VMName -NoStatus
...
Function ConnectTo-MyAzureVM{
    ...
    If($PSBoundParameters.ContainsKey('RequestJIT') ){
       ...
       Set-MyAzureVMJitPolicy -VMName $VMName
    }
